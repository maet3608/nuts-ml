...
We start with a tiny example, and randomly split a list of numbers
ranging from 0 to 9 into a training and a testing data set
with a ratio of 70%:
...
is required. Here it returns a tuple containing the split data sets.
...
``SplitRandom()`` returns the same split for each call. You can provide a 
...
.. note::
   ...
   repeatable results across Python versions use ``StableRandom``.
...
more accurate on the class with more samples. A common method to avoid this,
is to stratify the data by over- or under-sampling data based on their class
labels.
...
Obviously, this is an unbalanced data set. After stratification the samples
...
.. note::
   ...
   of samples. Use ``Shuffle`` to ensure random ordering.
...
As you can see, the training data is now balanced again. ``Shuffle(100)`` load 100
...
training loop. Here a typical example:
...
and shuffling randomizes the order of samples. This usually what you want but 
you can provide random number generators with specific seeds to change this 
default behavior.

For the building of batches and network training see the following sections
of the tutorial.
