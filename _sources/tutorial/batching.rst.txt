...
.. note::
   ...
   ``BuildBatch(2, prefetch=0)`` or ``BuildBatch(2, verbose=True)`` instead!
...
If there is no output/target data specified, e.g. for the prediction/inference phase,  
the generated batch is a list of inputs only:
...
out batches for debugging is not informative but ``BuildBatch`` provides a
``verbose`` flag that prints the shape and data type of the generated NumPy arrays
within the batch data structure.
The same code above but with ``verbose=True`` and the ``Print`` removed, produces
...
.. code:: Python
  ...
  >>> convert_label = ConvertLabel(LABELS, 1)
  >>> build_batch = (BuildBatch(2, verbose=True)
  ...                .input(0, 'image', 'float32')
  ...                .output(1, 'one_hot', 'uint8', N_CLASSES))  
  >>> samples >> read_image >> to_rgb >> convert_label >> build_batch >> Consume()    
  [[2x213x320x3:float32], [2x2:uint8]]
  [[1x213x320x3:float32], [1x2:uint8]]
...
.. code:: Python
  ...
  ...                .output(1, 'one_hot', 'uint8', N_CLASSES))  
  >>> samples >> read_image >> to_rgb >> convert_label >> build_batch >> Consume()    
  [[2x3x213x320:float32], [2x2:uint8]]
  [[1x3x213x320:float32], [1x2:uint8]]
